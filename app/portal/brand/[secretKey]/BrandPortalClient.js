          ...
          <div className="flex items-center gap-4 text-center sm:text-left flex-col sm:flex-row">
            {brand.imageUrl ? (
              <img 
                ...
                alt={brand.name} 
                className="w-16 h-16 rounded-xl object-cover border border-border cursor-pointer hover:border-primary transition-colors"
                      ...
                      <div key={p.id} className="bg-surface border border-border rounded-xl p-4 flex flex-col gap-3">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0 flex-1">
                            {p.imageUrl ? (
                              <img src={getOptimizedImageUrl(p.imageUrl, 80, 80)} alt={p.name} className="w-10 h-10 rounded-lg object-cover border border-border flex-shrink-0 cursor-zoom-in hover:brightness-95 transition-all duration-200" onClick={() => setLightboxImage({ url: p.imageUrl, name: p.name })} />
                            ) : (
                              ...
                              <div className="flex items-center gap-3">
                                {p.imageUrl ? (
                                  <img 
                                    ...
                                    alt={p.name} 
                                    className="w-8 h-8 rounded-lg object-cover border border-border cursor-pointer hover:border-primary transition-all flex-shrink-0"
