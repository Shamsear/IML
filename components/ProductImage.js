...
export default function ProductImage({
  ...
  return (
    <img
      ...
      alt={alt}
      className={`${sizes[size]} rounded-lg object-contain bg-background p-0.5 border border-border flex-shrink-0 ${onClick ? 'cursor-zoom-in hover:brightness-95' : ''} transition-all duration-200 ${className}`}
      onClick={onClick}
      onError={(e) => {
        if (e.target.src !== src) {
          e.target.src = src;
